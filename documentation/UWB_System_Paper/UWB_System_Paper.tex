...
In Figure \ref{fig:csvbarchart} the timing of one positioning cycle is illustrated.
...
The 250 ms round-trip time is a result of the anchor amount chosen.
The duration for each distance measurement, or ranging time, is set at a firm 50 ms due to limitations imposed by the processing logic, making it non-negotiable.
As a result, the total round-trip time is affected by the number of anchor devices utilized.
Adding more anchors enhances the amount of individual \acp{TWR}, leading to an increase in the overall round-trip time.
