...
\author{ \authorblockN{Sebastian Krebs} \authorblockN{University
  ...
  \authorblockA{Email: se391kre@htwg-konstanz.de} \and
  \authorblockN{Tom Herter} \authorblockN{University
  	of Applied Sciences HTWG Konstanz}
  \authorblockN{Alfred-Wachtel-Str.~8, 78462 Konstanz, Germany}
  \authorblockA{Email: \textcolor{red}{xxx}@htwg-konstanz.de}
  }
...
In Figure \ref{fig:csvbarchart}, the timing of one positioning cycle resembled that during the static tests conducted in section VI, where five anchor devices were utilized. The total time for position estimation is set to be 250 ms, allocating just 50 ms for each distance measurement.
\textcolor{green}{The 250 ms round-trip time is a result of various processes involved in the positioning cycle. The duration for each distance measurement, or ranging time, is set at a firm 50 ms due to limitations imposed by the processing logic, making it non-negotiable. As a result, the total round-trip time is affected by the number of anchor devices utilized. Adding more anchors enhances the intricacy of the calculation, leading to an increase in the overall round-trip time.}
